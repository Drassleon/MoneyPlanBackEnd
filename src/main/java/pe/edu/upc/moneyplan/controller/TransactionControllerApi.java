...
@CrossOrigin(methods = { RequestMethod.GET, RequestMethod.POST, RequestMethod.DELETE, RequestMethod.PUT,
		RequestMethod.OPTIONS }, allowedHeaders = { "Content-Type", "Authorization", "Access-Control-Allow-Headers",
				"X-Requested-With", "Access-Control-Max-Age", "Access-Control-Allow-Methods",
				"Access-Control-Allow-Origin" }, allowCredentials = "true")
public class TransactionControllerApi {
	...
	public List<TransactionDTO> findAll() {
		List<Transaction> rawTransactions = transactionService.findAll();
		if(rawTransactions.isEmpty())
		{
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,"List of transactions is empty!");
		}
		...
		for (Transaction rawTransaction : rawTransactions) {
			...
			if(rawTransaction.getCustomCategory()==null)
			{
				transaction.setCategoryName(rawTransaction.getDefaultCategory().getName());
			}
			else
			{
				transaction.setCategoryName(rawTransaction.getCustomCategory().getName());
			}
			
			transaction.setClientId(rawTransaction.getClient().getId());
			transaction.setDescription(rawTransaction.getDescription());
			transaction.setTimestamp(rawTransaction.getTimestamp());
			transaction.setTransactionType(rawTransaction.getTransactionType());
	...
	@ResponseBody
	public TransactionDTO findOne(@PathVariable("id") Long id) {
		Transaction rawTransaction = transactionService.findById(id);
		TransactionDTO transaction = new TransactionDTO();
		transaction.setAmount(rawTransaction.getAmount());
	...
	@ResponseBody
	public List<TransactionDTO> findByClientId(@PathVariable("clientId") Long clientId) {
		List<Transaction> rawTransactions = transactionService.findByClientId(clientId);
		if(rawTransactions.isEmpty())
		{
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,"List of transactions is empty!");
		}
	...
	@ResponseBody
	public List<SummaryDTO> ExpenseByCategory(@PathVariable("clientId") Long clientId, @PathVariable("year") int year,
		...
		for (Transaction expense : expenses) {
			...
			for (SummaryDTO summaryDTO : result) {
				if (summaryDTO.getCategoryName().equals(categoryname)
						&& (expense.getTransactionType() == 2 || expense.getTransactionType() == 3)
	...
	@ResponseBody
	public List<SummaryDTO> IncomeByCategory(@PathVariable("clientId") Long clientId, @PathVariable("year") int year,
			@PathVariable("month") int month) {
		List<SummaryDTO> result = new ArrayList<SummaryDTO>();
		...
		for (Transaction income : incomes) {
			...
			for (SummaryDTO summaryDTO : result) {
				if (summaryDTO.getCategoryName().equals(categoryname) && income.getTransactionType() == 1
						&& calendar.get(Calendar.MONTH) == month && calendar.get(Calendar.YEAR) == year) {
	...
	@ResponseBody
	public double ExpenseAverage(@PathVariable("clientId") Long clientId, @PathVariable("year") int year,
			@PathVariable("month") int month) {
	...
	@ResponseBody
	public double ExpensePerMonth(@PathVariable("clientId") Long clientId, @PathVariable("year") int year,
			@PathVariable("month") int month) {
		...
		for (Transaction expense : expenses) {
			Calendar calendar = Calendar.getInstance();
			calendar.setTime(expense.getTimestamp());
			if (expense.getTransactionType() == 2 && calendar.get(Calendar.YEAR) == year
	...
	@ResponseBody
	public double IncomePerMonth(@PathVariable("clientId") Long clientId, @PathVariable("year") int year,
			@PathVariable("month") int month) {
		...
		for (Transaction income : incomes) {
			Calendar calendar = Calendar.getInstance();
			calendar.setTime(income.getTimestamp());
			if (income.getTransactionType() == 1 && calendar.get(Calendar.YEAR) == year
					&& calendar.get(Calendar.MONTH) == month)
	...
	public HttpStatus create(@RequestBody TransactionDTO transaction) {
		...
		if (defaultCategory != null) {
			...
			CustomCategory customCategory = customCategoryService.findByName(transaction.getCategoryName(),
					client.getId());
	...
	public HttpStatus update(@RequestBody TransactionDTO transaction, @PathVariable("transactionId") long id) {
		...
		if (transactionAux != null) {
			transactionAux.setAmount(transaction.getAmount());
			transactionAux.setDescription(transaction.getDescription());

			DefaultCategory defaultCategory = defaultCategoryService.findByName(transaction.getCategoryName());
			if (defaultCategory != null) {
				transactionAux.setDefaultCategory(defaultCategory);
			} else {
				CustomCategory customCategory = customCategoryService.findByName(transaction.getCategoryName(),
						client.getId());
				if (customCategory != null) {
					transactionAux.setCustomCategory(customCategory);
				} else {
					customCategory = new CustomCategory();
					customCategory.setClient(client);
					customCategory.setName(transaction.getCategoryName());
			...
			transactionService.save(transactionAux);
			return HttpStatus.OK;
		}
